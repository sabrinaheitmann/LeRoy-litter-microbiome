...
#Idenify fwd and rev reads
in.path <- file.path("output/trim/16S/bact")
...
out.path <- file.path("output/dada/16S/bact")
...
#Filter and trim
trim <- filterAndTrim(path.fwd,filt.fwd,path.rev,filt.rev, 
                      maxN=0,
                      maxEE=c(2,2), truncQ=0, 
...
err.plot.fwd <- plotErrors(err.fwd, nominalQ=TRUE) + ggtitle(paste("Forward reads error model - ",'bact','16S'))
...
err.plot.rev <- plotErrors(err.rev, nominalQ=TRUE) + ggtitle(paste("Reverse reads error model - ",'bact','16S'))
...
#Make sequence table
#changed from merged to dada.fwd due to only fwd reads used
