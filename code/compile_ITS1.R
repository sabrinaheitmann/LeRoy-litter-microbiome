...
seqTab <- readRDS("output/dada/ITS1/seqTab.rds")
...
dir.create("output/compile/ITS1.scratch")
#write temp file
writeXStringSet(seqs,file="output/compile/ITS1.scratch/tmp.fasta",width=1000)

#change out to Salix genome if needed
#system("/home/busbylab/miniconda3/bin/bowtie2 --threads 10 -x data/Ptri_genome/CopyOfPtri.v.3.db -f output/compile/ITS1.scratch/tmp.fasta --un output/compile/ITS1.scratch/noHost.fa --al output/compile/ITS1.scratch/host.fa --very-sensitive-local")

#changed to tmp.fasta from noHost.fa
seqs.noHost <- readDNAStringSet("output/compile/ITS1.scratch/tmp.fasta")
...
#changed to tmp.fasta from noHost.fa
ITSx.flags <- paste("-i output/compile/ITS1.scratch/tmp.fasta",
                    "-t 'fungi'",
                    "--preserve T",
                    "--complement F",
                    "--summary T",
                    "--cpu 10",
                    "-o output/compile/ITS1.scratch/ITSx",
                    "--only_full T",
                    "-E 1e-2")
...
seqs.ITS <- readDNAStringSet("output/compile/ITS1.scratch/ITSx.ITS1.fasta") %>% .[.@ranges@width > 75]
...
saveRDS(phy,"output/phy_ITS1/phy_ITS1.rds")

#Save components for possible manual inspection
otu_table(phy) %>% write.csv("output/phy_ITS1/ITS1.OTU.table.csv")
tax_table(phy) %>% write.csv("output/phy_ITS1/ITS1.taxonomy.table.csv")
sample_data(phy) %>% write.csv("output/phy_ITS1/ITS1.sample.data.csv")
